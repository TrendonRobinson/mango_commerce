...
class SignUp(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        if created_user.is_valid():
            ...
            created_user.save()
            return Response(
                {"user": created_user.data}, status=status.HTTP_201_CREATED
            )
        else:
...
class SignIn(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        if user is not None:
            ...
            return Response(
                {"user": {"id": user.id, "email": user.email, "token": token.key}}
            )
        else:
            return Response(
...
class ChangePassword(generics.UpdateAPIView):
    def patch(self, request):
        ...
        if not user.check_password(old_pw):
            return Response(
                {"msg": "Wrong password"}, status=status.HTTP_422_UNPROCESSABLE_ENTITY
            )
